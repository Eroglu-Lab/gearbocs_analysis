---
title: "20240715_vamp2_inhib_puncta_analyzer"
...
data_cleaned_1 <- read_csv("Vamp2 Inhi.csv")
data_cleaned_2 <- read_csv("Vamp2 INhi-2.csv")
data_cleaned_3 <- read_csv("Vamp2 Inhi-3.csv")
...
data_cleaned[which(grepl("*VAMP2TRAP*", data_cleaned$Image)),]$condition <- "Vamp2 Trap"
...
ggplot(plot.df, aes(x = condition, y = `mean(normalized_count)`)) + 
  ...
  labs(y = "Fold change in synapse number \n (colocalization of VGAT and Gephyrin)") +
  scale_color_manual(values = c("black", "black")) + theme(legend.position = "none") + 
  theme_bw()
  


# ggsave("vamp2_inhib_puncta_analyzer_v2.pdf", width = 3, height = 4)
# ggsave("vamp2_inhib_puncta_analyzer_v2.png", width = 3, height = 4)
